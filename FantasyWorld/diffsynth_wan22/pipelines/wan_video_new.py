...
class WanVideoPipeline(BasePipeline):
    ...
    def __call__(
        ...
        # Camera control
        camera_control_direction: Optional[Literal['move_right', 'pan_right', 'orbit_left', 'pull_out', 'move_left', 'orbit_right', 'pan_left', 'push_in']] = None,
        ...
        inputs_shared = {
            ...
            "camera_control_direction": camera_control_direction, "camera_control_speed": camera_control_speed, "camera_control_origin": camera_control_origin, "cameras_interp":cameras_interp,
        ...
        for progress_id, timestep in enumerate(progress_bar_cmd(self.scheduler.timesteps)):
            ...
            # print("progress_id = {}, timestep = {}, switch_DiT_boundary * self.scheduler.num_train_timesteps = {}".format(progress_id, timestep, switch_DiT_boundary * self.scheduler.num_train_timesteps))
            if timestep.item() < switch_DiT_boundary * self.scheduler.num_train_timesteps and self.dit2 is not None and not models["dit"] is self.dit2:
                print("self.load_models_to_device(self.in_iteration_models_2)")
    ...
    def one_step(
        ...
        file_name=None,
        save_path=None,
        mid_images_list=None,
        mid_images_id_list=None,
        progress_bar_st=None,
        min_timestep_boundary=0.0,
        max_timestep_boundary=1.0,
        # switch_DiT_boundary=0.358,
        models=dict(),
    ):
        # Parameter check
        ...
        # # Scheduler
        # self.scheduler.set_timesteps(num_train_steps, training=True)

        # Initialize noise
        length = (num_frames - 1) // 4 + 1
        shape = (1, self.vae.model.z_dim, length, height // self.vae.upsampling_factor, width // self.vae.upsampling_factor)
        noise = self.generate_noise(shape, seed=seed, rand_device=rand_device, device=self.device, torch_dtype=self.torch_dtype)
        
        # Prepare inputs for units
        inputs_posi = {
            "prompt": prompt,
            "positive": True,
        }
        ...
        inputs_shared = {
            ...
            "mid_images_list": mid_images_list,
            "mid_images_id_list": mid_images_id_list,
            "tea_cache_l1_thresh": tea_cache_l1_thresh,
            "tea_cache_model_id": tea_cache_model_id,
            "num_inference_steps": num_inference_steps,
        }

        # Run units to prepare inputs
        for unit in self.units:
            inputs_shared, inputs_posi, inputs_nega = self.unit_runner(unit, self, inputs_shared, inputs_posi, inputs_nega)

        # Get latents from units and prepare clean_latents
        clean_latents = None
        print("Line 676", "latents" not in inputs_shared)
        if "latents" not in inputs_shared:
            inputs_shared["latents"] = noise
        else:
            # If input_video was provided, save clean_latents before adding noise
            if input_video is not None:
                clean_latents = inputs_shared.get("input_latents", inputs_shared["latents"]).clone()
                timestep_id = torch.randint(int(min_timestep_boundary * self.scheduler.num_train_timesteps), 
                                            int(max_timestep_boundary * self.scheduler.num_train_timesteps), (1,))
                timestep = self.scheduler.timesteps[timestep_id].to(dtype=self.torch_dtype, device=self.device)
                inputs_shared["latents"] = self.scheduler.add_noise(clean_latents, noise, timestep)
            else:
                # No input_video, latents is just noise
                pass

        # Get timestep if not already set (when input_video is None)
        ...
        # Inference
        timestep = timestep.unsqueeze(0).to(dtype=self.torch_dtype, device=self.device)
        noise_pred_posi = self.model_fn(**models, **inputs_shared, **inputs_posi, timestep=timestep, **extra_kwargs)
        
        if cfg_scale != 1.0:
            noise_pred_nega = self.model_fn(**models, **inputs_shared, **inputs_nega, timestep=timestep, **extra_kwargs)
            noise_pred = noise_pred_nega + cfg_scale * (noise_pred_posi - noise_pred_nega)
        else:
            noise_pred = noise_pred_posi

        # Scheduler step
        latents_denoise = self.scheduler.step(noise_pred, timestep, inputs_shared["latents"])

        # Cache is saved inside model_fn_wan_video at block 15, no need to return values
        # Just return None or minimal values for compatibility
        return None
...
class WanVideoUnit_ImageEmbedderVAE(PipelineUnit):
    def __init__(self):
        super().__init__(
            input_params=("input_image", "end_image", "num_frames", "height", "width", "tiled", "tile_size", "tile_stride", "mid_images_list", "mid_images_id_list"),
            onload_model_names=("vae",)
        )

    def process(self, pipe: WanVideoPipeline, input_image, end_image, num_frames, height, width, tiled, tile_size, tile_stride, mid_images_list=None, mid_images_id_list=None):
        ...
        if mid_images_list is not None and mid_images_id_list is not None:
            for mid_image, mid_image_id in zip(mid_images_list, mid_images_id_list):
                mid_image = pipe.preprocess_image(mid_image.resize((width, height))).to(pipe.device)
                vae_input[:, mid_image_id:mid_image_id+1] = mid_image.transpose(0,1)
                msk[:, mid_image_id:mid_image_id+1] = 1
...
class WanVideoUnit_FunCameraControl(PipelineUnit):
    def __init__(self):
        super().__init__(
            input_params=("height", "width", "num_frames", "camera_control_direction", "camera_control_speed", "camera_control_origin", "cameras_interp", "latents", "input_image", "tiled", "tile_size", "tile_stride"),
            onload_model_names=("vae",)
        )

    def process(self, pipe: WanVideoPipeline, height, width, num_frames, camera_control_direction, camera_control_speed, camera_control_origin, cameras_interp, latents, input_image, tiled, tile_size, tile_stride):
        if camera_control_direction is None:
            return {}
        pipe.load_models_to_device(self.onload_model_names)
        camera_control_plucker_embedding = pipe.dit.control_adapter.process_camera_coordinates(
            camera_control_direction, num_frames, height, width, camera_control_speed, camera_control_origin, cameras_interp)
    ...
    **kwargs,
    ...
    if tea_cache_update:
        ...
        for block_id, block in enumerate(dit.blocks):
            ...
            # Save cache at block 15 (16th block)
            if "file_name" in kwargs.keys() and "save_path" in kwargs.keys() and block_id == 15:
                tensor_to_save = x.detach().cpu().contiguous()
                save_file({"hidden_states": tensor_to_save}, os.path.join(kwargs['save_path'], f"{kwargs['file_name']}_{int(timestep.item())}.safetensors"))
                return None
